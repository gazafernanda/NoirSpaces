...
function Home() {
  ...
  return (
    ...
    >
      ...
      {/* Main Content */}
      <main className="min-h-screen flex items-center justify-center py-20 px-12 relative z-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 w-full max-w-[1400px]">
          {/* Hero Section */}
          <section
            className={`flex flex-col justify-center items-start text-left lg:pr-16 transition-all duration-1000 delay-200 ${isLoaded
          ...
          >
            ...
            <h1
              className="font-['Playfair_Display'] text-6xl lg:text-8xl font-light leading-tight tracking-[-0.03em] mb-12"
            >
              <span className="block font-extralight text-[rgba(255,255,255,0.6)]">
                {currentSlide.titleLight}
              </span>
              <span className="block font-semibold drop-shadow-2xl">
          ...
          >
            ...
            >
              {images.map((image, index) => (
                <article
                  key={image.id}
                  className={`relative w-48 h-[350px] rounded-2xl overflow-hidden cursor-pointer transition-all duration-500 shadow-[0_20px_50px_rgba(0,0,0,0.5)] group ${index === activeImage
                    ? "w-60 shadow-[0_30px_60px_rgba(0,0,0,0.7)] scale-105 border border-[rgba(255,255,255,0.1)]"
                ...
                >
                  ...
                  <div
                    className="absolute bottom-0 left-0 right-0 p-8 z-20 opacity-0 transform translate-y-4 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0"
                  >
                    <span className="text-sm font-medium tracking-[0.1em] uppercase text-white shadow-black drop-shadow-md">
                      {image.title}
                    </span>
                  </div>
                </article>
